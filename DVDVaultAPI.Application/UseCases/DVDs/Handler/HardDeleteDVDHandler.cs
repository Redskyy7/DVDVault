...
public class HardDeleteDVDHandler : IHardDeleteDVDHandler
{
    private readonly IDVDRepository _dvdRepository;
    private readonly IUnitOfWork _unitOfWork;

    public HardDeleteDVDHandler() { }

    public HardDeleteDVDHandler(IDVDRepository dvdRepository, IUnitOfWork unitOfWork)
    {
        _dvdRepository = dvdRepository;
        _unitOfWork = unitOfWork;
    }

    public async Task<IResponse> Handle(HardDeleteDVDRequest request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            return await HardDeleteDVD(dvdDB, cancellationToken);
        }
        catch (Exception ex)
        {
    ...
    private async Task<IResponse> HardDeleteDVD(DVD dvdDB, CancellationToken cancellationToken)
    {
        _unitOfWork.BeginTransaction();

        var deleted = await _dvdRepository.HardDelete(dvdDB);
        if (deleted == false)
